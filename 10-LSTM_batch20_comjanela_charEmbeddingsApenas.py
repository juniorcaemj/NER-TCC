...
model = Sequential()
model.add(Embedding(output_dim=char_embedding_size, input_dim=char_vocab_size, input_length=n_in*max_charlen_padded, init='glorot_uniform'))
model.add(Reshape((n_in, max_charlen_padded, char_embedding_size)))
model.add(TimeDistributed(Convolution1D(nb_filter=n_filters, filter_length=char_window, border_mode='valid')))
model.add(TimeDistributed(MaxPooling1D(pool_length=max_charlen)))
model.add(Reshape((n_in, n_filters)))
model.add(LSTM(output_dim=n_hidden, init='glorot_uniform', activation='tanh',
                batch_input_shape=(None,n_in,char_embedding_size),return_sequences=True),)
...
exit(0)
'''
#model.load_weights('modelos_treinados/7-LSTM_batch20_comjanela_charEmbeddings/parciais/modelo_parcial1.h5')
...
for epoch in xrange(number_of_epochs):
    ...
    #Train for 1 epoch
    hist = model.fit(train_x_char, train_y_cat, nb_epoch=1, batch_size=minibatch_size,
                     verbose=True, shuffle=True)
    print "%.2f sec for training\n" % (time.time() - start_time)

    #Save temporary model
    model.save_weights('modelos_treinados/10-LSTM_batch20_comjanela_charEmbeddingsApenas/parciais/modelo_parcial'+str(epoch+1)+'.h5',overwrite=True)

    ### Testing ###
    a = model.predict_classes(test_x_char)
...
#Save final model
model.save_weights('modelos_treinados/10-LSTM_batch20_comjanela_charEmbeddingsApenas/modelo_final.h5',overwrite=True)